...
export default function WorkoutEditorPage() {
  ...
  const { data: days = [], isLoading } = useQuery<WorkoutDay[]>({
    queryKey: queryKeys.workouts,
    queryFn: () => api.get<WorkoutDay[]>("/api/workouts/all"),
  });

  const saveDay = useApiMutation("/api/workouts", "POST");

  const handleSave = async (day: WorkoutDay) => {
    await saveDay.mutateAsync(day as unknown as Record<string, unknown>);
    await invalidate(queryKeys.workouts);
    setEditing(null);
  };

  const handleDelete = async (dow: number) => {
    await api.delete(`/api/workouts/${dow}`);
    await invalidate(queryKeys.workouts);
  };
